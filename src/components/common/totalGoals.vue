<template>
	<div class="totalGoals">
					...
					<li v-for="(items,index) in leftBlock" class="match_li">
						...
						<div class="match_detail">
							<div class="one_info match_select">
								<p class="match_date" style="margin-top:17px;">{{items.date}}</p>
							...
							<p class="match_titel">
								...
								<!-- 赛事投注选项区域 -->
								<ul style="font-size:0;margin:10px 0;">
									<li v-for="(innerItem,innerIndex) in items.liveSp" class="match_block_spf" :class="{'actives': curIndexs.indexOf(index + '' + innerIndex) >= 0,'lottery_type_btn':modeTab=='TD','game_type_btn':modeTab=='GAME'}" @touchstart="ChooseRed(index,items,innerItem,innerIndex)">
							...
							<ul class="have_chosen">
								<li class="betting_block">
									...
									<ul style="text-align:center;">
										<li class="detail_home" v-for="innerTemp in temp.liveSp">{{innerTemp.QS}}</li>
									</ul>
								</li>
							</ul>
						</li>
		...
		<tipsExceed v-if="tipsShowExceed" title="最大比赛场次不能超过6场"></tipsExceed>
